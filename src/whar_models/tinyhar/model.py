...
class TinyHAR_Model(nn.Module):
    def __init__(
        ...
        for i in range(nb_conv_layers):
            ...
            if i%2 == 1:
                layers_conv.append(nn.Sequential(
                    nn.Conv2d(in_channel, out_channel, (filter_size_list[i], 1),(1,1)),
                    nn.ReLU(inplace=True),#))#,
...
class TinyHAR_Wrapper(ModelWrapper):
    NAME = "TinyHAR"
    display_name = "TinyHAR"
    color = "#f8961e"
    ARCHITECTURE = "Conv2D + cross-channel interaction + temporal aggregation"
    ...
    def __init__(
        ...
        ts_len: int = 128,
        filter_num: int = 32,
        cross_channel_aggregation_type: str = "FC",
        **kwargs,
    ) -> None:
